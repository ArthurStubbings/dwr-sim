...
Routh/Russel film-P\'{e}clet framework — to an idealised plain-weave unit cell,
...
film P\'{e}clet number; the functional polymer is therefore swept toward the
...
carrier/surfactant phase ($r_{S} \sim \SI{40}{\nano\metre}$).
...
The competition is governed by the film P\'{e}clet number,
...
In a woven textile this drying physics operates on a non-uniform film:
capillary pressure preferentially draws the dispersion into the
high-curvature inter-yarn interstices, leaving a thicker residual film
there than on the exposed yarn crowns.
...
\begin{itemize}\setlength\itemsep{1pt}
  \item Conservative finite-volume discretisation on fixed cell centres —
        $\int q\,\mathrm{d}x$ is conserved to $\lesssim 10^{-12}$ (verified
        over 12 P\'{e}clet--step combinations).
...
\begin{enumerate}\setlength\itemsep{1pt}
  ...
  \item \textbf{Contact angle.} A Cassie--Baxter mixing rule between the
        fully-coated ($\theta_{\mathrm{DWR}}=\SI{105}{\degree}$) and
        bare-yarn ($\theta_{\mathrm{bare}}=\SI{65}{\degree}$) states yields
        the local effective contact angle.
...
\begin{figure}[H]
  ...
  \caption{Stratification index (difference in centre-of-mass height between
    ...
    qualitative behaviour of the Routh/Russel and Fortini
    binary-stratification frameworks.}
  \label{fig:stratmap}
\end{figure}
...
at high $\mathrm{Pe}$ the large species is swept toward the film surface
during drying and then buried against the substrate as the film collapses —
exactly the condition that degrades beading.
...
\begin{figure}[H]
  ...
  \caption{Plain-weave unit-cell geometry. \textit{Left:} surface elevation
    ...
    on crowns to $\approx\SI{135}{\micro\metre}$ in interstices.
    \textit{Right:} local film P\'{e}clet number; interstices are the
    highest-Pe zones.}
  \label{fig:unitcell}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{Spatial deposition results at moderate polymer loading
    ($\phi_{0}=0.045$). \textit{Left:} effective DWR coverage.
    \textit{Right:} predicted contact-angle map (Cassie--Baxter). Valley
    contact angles ($\approx\SI{94}{\degree}$) exceed crown contact angles
    ($\approx\SI{86}{\degree}$) — the crowns are starved of functional
...
At moderate polymer loading the contact angle is \emph{higher in the
valleys} than on the crowns ($\approx\SI{94}{\degree}$ vs.\
$\approx\SI{86}{\degree}$).
...
\begin{figure}[H]
  ...
  \caption{Beading index over the 2D process space of polymer loading
    $\phi_{0}$ and evaporation rate. High index (bright) requires adequate
    loading \emph{and} slow drying. Dashed contour marks the $0.75$
    threshold.}
  \label{fig:procwin}
\end{figure}
...
($\phi_{0} \gtrsim 0.07$) combined with slow, cool drying (low evaporation
rate), giving beading index $>0.75$.

% ── 4. Implications ──────────────────────────────────────────────────────────
\section{Implications for Materials Development and QA}

\textbf{Formulation.}\;
Polymer loading is the most robust lever: increasing $\phi_{0}$ from
\num{0.045} to \num{0.09} raises the beading index from $\approx 0.35$ to
$\approx 0.75$ at slow drying rates.
...
A physics-based model has been built and verified against the known
...
drives a high-P\'{e}clet burial of functional polymer, starving the exposed
yarn crowns — is mechanistically clear, quantified, and reproducible in a
locked computational test.
The process-window output provides an actionable, two-parameter design
guide: adequate polymer loading ($\phi_{0} \gtrsim 0.07$) combined with
slow, cool drying is both necessary and sufficient for good predicted
beading performance.
The immediate next step to ground this model is a two-temperature
spray/bead test, which would anchor the relative beading index to absolute
contact-angle measurements and close the loop between computation and
experiment.
The model is openly available with full source code, 12 passing
verification tests, and an interactive dashboard for real-time parameter
exploration.
...
All results are predictions of a computational model, verified against the
known theoretical behaviour of the colloidal-film stratification literature.
No experimental DWR measurements were performed.\\[3pt]
\textbf{Scientific basis.}
Routh \& Russel (evaporation-driven colloidal-film drying, film P\'{e}clet
framework);
